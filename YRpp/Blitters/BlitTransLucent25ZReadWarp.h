...
DEFINE_BLITTER(BlitTransLucent25ZReadWarp)
{
public:
	inline explicit BlitTransLucent25ZReadWarp(T* data, WORD mask) noexcept
	{
		PaletteData = data;
		Mask = mask;
	}

	virtual ~BlitTransLucent25ZReadWarp() override final = default;
	...
	virtual void Blit_Copy(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp) override final
	{
		...
		while (len--)
		{
			...
			{
				if (byte idx = *src)
					*dest = (Mask & (dest[warp] >> 2)) + 3 * (Mask & (PaletteData[idx] >> 2));
			}
			++src;
			++dest;
