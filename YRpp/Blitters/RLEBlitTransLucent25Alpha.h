...
DEFINE_RLE_BLITTER(RLEBlitTransLucent25Alpha)
{
public:
	inline explicit RLEBlitTransLucent25Alpha(T* data, WORD mask, int shadecount) noexcept
	{
		PaletteData = data;
		Mask = mask;
	...
	virtual ~RLEBlitTransLucent25Alpha() override final = default;
	...
	virtual void Blit_Copy(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
		...
		{
			dest = (Mask & (dest >> 2)) + 3 * (Mask & (PaletteData[srcv | adata[*abuf]] >> 2));
		};
