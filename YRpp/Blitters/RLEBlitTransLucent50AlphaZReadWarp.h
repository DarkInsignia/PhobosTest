...
DEFINE_RLE_BLITTER(RLEBlitTransLucent50AlphaZReadWarp)
{
public:
	inline explicit RLEBlitTransLucent50AlphaZReadWarp(T* data, WORD mask, int shadecount) noexcept
	{
		PaletteData = data;
		Mask = mask;
	...
	virtual void Blit_Copy(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
		...
		auto handler = [this](T& dest, byte srcv, int zbase, WORD zbufv, byte zadjustv, WORD abufv)
		{
			...
			if (zval < zbufv)
				dest = (Mask & ((&dest)[warp] >> 1)) + (Mask & (PaletteData[srcv | adata[*abuf]] >> 1));
		};
